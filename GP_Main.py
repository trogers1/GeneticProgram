...
@authors: Savannah Rogers, Taylor Rogers
"""
...
def same_fit_smaller_size(subset, proximity):
    ...
    for i in range(1, len(subset)):  # for all the other individuals in tournament
        # if individual's fitness is within range
        if subset[i].fitness > fitcheckmin and subset[i].fitness < fitcheckmax:
            topfits.append(subset[i])  # add that individual to topfits list
            # add the size of that individual to size list
            sizes.append(subset[i].get_tree_size())
...
def population_reproduction(population, pop_size, tournament_size, mut_prob, leaf_mut_prob, leaf_probability, full_df, total_pres, total_abs, variables):
    ...
    if choices < len(population):
        winner2, choices2 = same_fit_smaller_size(
            population[choices:], .000002)
    else:
    ...
    while len(newpop) <= pop_size:  # 2 individuals are appended to the population each generation
        ...
        # mutate each
        mutate_ind(mut_prob, parent1, variables, leaf_mut_prob,
                   full_df, total_pres, total_abs)
        mutate_ind(mut_prob, parent2, variables, leaf_mut_prob,
                   full_df, total_pres, total_abs)
        # perform crossover
        offspring1, offspring2 = cross_over(
            parent1, parent2, leaf_probability, full_df, total_pres, total_abs, variables)
...
def cross_over(tree1, tree2, leaf_probability, full_df, total_pres, total_abs, variables):
    ...
    # if both node1 and node2 are not root (so they do have parents)
...
def mutate_ind(mut_prob, ind, variables, leaf_mut_prob, full_df, total_pres, total_abs):
    ...
    if ind.left:
        mutate_ind(mut_prob, ind.left, variables, leaf_mut_prob,
                   full_df, total_pres, total_abs)
    if random.randint(1, 100) <= (100*mut_prob):
        if ind.position == "left":
            ind.parent.left = mutate_node(ind, variables)
        if ind.position == "right":
            ind.parent.right = mutate_node(
                ind, variables)  # root node is not mutated
    if ind.right:
        mutate_ind(mut_prob, ind.right, variables,
                   leaf_mut_prob, full_df, total_pres, total_abs)
...
def mult_gen():
    ...
    pop, fit, sd, best_ind, best_fit = pop_generate(
        raw_data, data_pres, data_abse, pop_size, variables, max_height)
    ...
    for i in range(1, generations):
        ...
        pop, avg_pop_fit, sd_pop_fit, best_ind, best_fit = population_reproduction(
            pop, pop_size, tournament_size, mut_prob, leaf_mut_prob, leaf_probability, raw_data, data_pres, data_abse, enviro_vars)
        ...
        best_fits.append(best_fit)
        generation.append(i)
        print("Process {}: .........................{}".format(PROC_NUM, i))
        log("Best individual: ")
        if log.is_verbose:
...
if __name__ == "__main__":
    for arg in sys.argv:
        ...
        if "--proc_num=" in arg:
            PROC_NUM = arg
            PROC_NUM = PROC_NUM.split("=")[1]
            sys.argv.remove(arg)
    RAW_DATA = pandas.read_csv(sys.argv[1])
    VARS = RAW_DATA.loc[:, RAW_DATA.columns !=
                        'presence']  # all columns except presence
    for i in list(VARS):
